...
<p>There is a Chef server which stores each of these cookbooks and as a new chef client node checks in with the server, recipes are sent to instruct the node on how to configure itself.</p>

<p>The client will then check in every now and then to make sure that no changes have occurred, and nothing needs to change. If it does, then the client deals with it. Patches and updates can be rolled out over the entire infrastructure by changing the recipe, thereby eliminating the need to interact with each machine individually.</p>
